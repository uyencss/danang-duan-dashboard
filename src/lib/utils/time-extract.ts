...
export function extractTimeFields(date: Date | string) {
  ...
  // Force use of Vietnam timezone for reporting fields
  const vnTime = d.toLocaleString("en-US", { timeZone: "Asia/Ho_Chi_Minh" });
  const localD = new Date(vnTime);

  // Year
  const nam = localD.getFullYear();
  
  // Month (1-12)
  const thang = localD.getMonth() + 1;
