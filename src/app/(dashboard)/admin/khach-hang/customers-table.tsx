  ...
  const handleExport = () => {
    const exportData = table.getFilteredRowModel().rows.map(row => {
      ...
      return {
        ...
        "Email": kh.email || "",
        "Đầu mối tiếp cận": kh.danhSachDauMoi?.length ? kh.danhSachDauMoi.map((d: any) => d.hoTen).join(", ") : (kh.dauMoiTiepCan || ""),
        "SĐT Đầu mối": kh.danhSachDauMoi?.length ? kh.danhSachDauMoi.map((d: any) => d.soDienThoai).filter(Boolean).join(", ") : (kh.soDienThoaiDauMoi || ""),
        "Ngày sinh Đầu mối": kh.danhSachDauMoi?.length ? kh.danhSachDauMoi.map((d: any) => d.ngaySinh ? new Date(d.ngaySinh).toLocaleDateString("vi-VN") : "").filter(Boolean).join(", ") : (kh.ngaySinhDauMoi ? new Date(kh.ngaySinhDauMoi).toLocaleDateString("vi-VN") : ""),
        "Lãnh đạo đơn vị": kh.danhSachLanhDao?.length ? kh.danhSachLanhDao.map((d: any) => d.hoTen).join(", ") : (kh.lanhDaoDonVi || ""),
        "SĐT Lãnh đạo": kh.danhSachLanhDao?.length ? kh.danhSachLanhDao.map((d: any) => d.soDienThoai).filter(Boolean).join(", ") : (kh.soDienThoaiLanhDao || ""),
        "Ngày sinh Lãnh đạo": kh.danhSachLanhDao?.length ? kh.danhSachLanhDao.map((d: any) => d.ngaySinh ? new Date(d.ngaySinh).toLocaleDateString("vi-VN") : "").filter(Boolean).join(", ") : (kh.ngaySinhLanhDao ? new Date(kh.ngaySinhLanhDao).toLocaleDateString("vi-VN") : ""),
